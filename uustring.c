...
static stringmap messages[] = {
    ...
    {S_SHORT_BINHEX,
     "BinHex encoded file %s ended prematurely (%ld bytes left)"},
    ...
    {S_STRIPPED_SETUID,
     "Stripped setuid/setgid bits from target file %s mode %d"},
    {S_DATA_SUSPICIOUS,
     "Data looks suspicious. Decoded file might be corrupt."},
    ...
    {S_MIME_B_NOT_FOUND,
     "Boundary expected on Multipart message but found EOF"},
...
char *uustring(int codeno)
{
    ...
    UUMessage(uustring_id, __LINE__, UUMSG_ERROR,
	      "Could not retrieve string no %d", codeno);
