...
size_t UUbhwrite(char *ptr, size_t sel, size_t nel, FILE * file)
{
	...
	count = UUbhdecomp(ptr, tmpstring, &lc, &rpc, nel, 256, &opc);
...
static uulist *UU_smparts_r(uulist *addit, int pass)
{
	...
	if (flag == 0 || (pass == 0 && count > 0) || (pass == 1 && count > 5)) {
	    iter = iter->NEXT;
	    continue;
	}
...
int UUEXPORT UUSmerge(int pass)
{
