...
char *uuscan_id = "$Id: uuscan.c,v 1.31 2001/06/06 19:21:52 fp Exp $";
...
static int IsLineEmpty(char *data)
{
    if (data == NULL)
	return 0;
...
static char *ScanHeaderLine(FILE *datei, char *initial)
{
    ...
    while (llength && isspace((int)*(ptr - 1))) {
 ...
 */
static char *ParseValue(char *attribute)
{
    ...
    while ((isalnum((int)*attribute) || *attribute == '_')
	   && *attribute != '=')
	attribute++;

    while (isspace((int)*attribute))
	attribute++;
	...
	*ptr = '\0';
	...
	while (*attribute && !isspace((int)*attribute) &&
	       *attribute != '(' && *attribute != ')' &&
	       *attribute != '<' && *attribute != '>' &&
	       *attribute != '@' && *attribute != ',' &&
	       *attribute != ';' && *attribute != ':' &&
...
static headers *ParseHeader(headers * theheaders, char *line)
{
    char **variable = NULL;
    char *value = NULL, *ptr = NULL, *thenew = NULL;
    int delimit = 0, length = 0;
	...
	if ((ptr = _FP_stristr(line, "boundary")) != NULL) {
		...
		if (theheaders->boundary)
		    free(theheaders->boundary);
	...
	while (length && isspace((int)*(ptr - 1))) {
	    ptr--;
	    length--;
	}
	*ptr = '\0';
...
static int IsKnownHeader(char *line)
{
...
ScanData(FILE * datei, char *fname, int *errcode,
	 char *boundary, int ismime, int checkheaders, fileread * result)
{
    char *line = uuscan_sdline, *bhds1 = uuscan_sdbhds1, *bhds2 =
	uuscan_sdbhds2;
    static char *ptr = NULL, *p2 = NULL, *p3 = NULL, *bhdsp = NULL, bhl;
    ...
    long preheaders = 0, oldposition = 0;
    size_t dcc, bhopc;
    ...
    *errcode = UURET_OK;
	...
	if (strncmp(line, "begin ", 6) == 0 ||
	    ...
	    while (isspace
		   ((int)result->filename[strlen(result->filename) - 1]))
	...
	if (boundary == NULL && !ismime && !uu_more_mime) {
		...
		if (*p2 == '"') {
		    ...
		    while (*p2 && isprint((int)*p2) && !isspace((int)*p2)
			   && *p2 != ';')
			p2++;
		}
		if (p2 && *p2 && p2 != ptr) {
		...
		if (line[0] == '-' && line[1] == '-') {
			...
			&& (encoding == B64ENCODED
			    || result->uudet == B64ENCODED)) {
			if (encoding == B64ENCODED && result->uudet == 0
			    && (haddh || hadct)) {
			    result->uudet = encoding;
			    encoding = dflag = 0;
			}
	...
	if (!(result->uudet)) {
	    ...
	    isb64[iscnt % 10] =
		(UUValidData(line, B64ENCODED, &bhflag) == B64ENCODED);
	    isuue[iscnt % 10] =
		(UUValidData(line, UU_ENCODED, &bhflag) == UU_ENCODED);
	    isxxe[iscnt % 10] =
		(UUValidData(line, XX_ENCODED, &bhflag) == XX_ENCODED);
	    isbhx[iscnt % 10] =
		(UUValidData(line, BH_ENCODED, &bhflag) == BH_ENCODED);
...
fileread *ScanPart(FILE * datei, char *fname, int *errcode)
{
    ...
    int bhflag = 0, begflag = 0, vflag = 0, blen = 0, res = 0;
    long preheaders = 0, prevpos = 0, preenc = 0, before = 0;
	...
	*errcode = UURET_NOMEM;
	...
	if (!uu_more_mime &&
	    ...
	    sstate.envelope.ctenc == NULL && IsKnownHeader(line)) {
	    /*
	     * see above
	     */
	    if (_FP_fgets(line, 255, datei) == NULL) {
	...
	if (uu_fast_scanning && mssdepth == 0) {
		...
		if ((sstate.envelope.subject != NULL
		     && result->subject == NULL) || result->filename == NULL
		    || result->sfname == NULL) {
		    *errcode = UURET_NOMEM;
		}
	...
	while (!feof(datei)) {
		...
		strncmp(line + 2,
			multistack[mssdepth - 1].envelope.boundary,
			blen) == 0)
		break;
	...
	if (result->uudet) {
		...
		if (line[0] == 'C' && line[1] == 'o' &&
			...
			if (strcmp(ptr1, multistack[res].envelope.boundary) ==
			    0)
			    break;
		    if (res < mssdepth)
			break;
		...
		if ((res =
		     ScanData(datei, fname, errcode, NULL, 1, 1,
			      result)) == -1) {
		...
		if (localenv.fname) {
		    _FP_free(result->filename);
		    if ((result->filename =
			 _FP_strdup(localenv.fname)) == NULL)
			*errcode = UURET_NOMEM;
		}
		else if ((result->uudet == QP_ENCODED
			  || result->uudet == PT_ENCODED)
	...
	if (line[0] == '-' && line[1] == '-' &&
		...
		*errcode = UURET_NOMEM;
	    ...
	    if ((res =
		 ScanData(datei, fname, errcode, NULL, 1, 1, result)) == -1) {
		/* oops, something went wrong */
		sstate.isfolder = 0;
		sstate.ismime = 0;
		UUkillfread(result);
	...
	if (localenv.mimevers && (strcmp(localenv.mimevers, "1.0") == 0) &&
	...
	if (result->origin)
	    _FP_free(result->origin);
	result->origin = _FP_strdup(localenv.from);

	if (result->subject)
	    _FP_free(result->subject);
	result->subject = _FP_strdup(localenv.subject);
	 ...
	 */
	if (uu_fast_scanning
	    && (result->uudet != 0 || sstate.envelope.partno != 1)) {
	    /* do nothing */
	    res = 0;
	}
	...
	if (localenv.fname) {
		...
		*errcode = UURET_NOMEM;
	    ...
	    _FP_free(result->filename);
	    if ((result->filename =
		 _FP_strdup(sstate.envelope.fname)) == NULL)
		*errcode = UURET_NOMEM;
	}
	else if ((result->uudet == QP_ENCODED || result->uudet == PT_ENCODED)
		 && result->filename == NULL) {
	...
	if (line[0] == '-' && line[1] == '-' &&
	    ...
	    if ((ptr1 == NULL
		 || (*(ptr1 + 2) != '\012' && *(ptr1 + 2) != '\015')) && ptr2
		&& _FP_strnicmp(ptr2, "Content-", 8) == 0) {
		/*
