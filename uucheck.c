...
char *uucheck_id = "$Id: uucheck.c,v 1.12 2001/06/06 18:21:47 fp Exp $";
...
static char *UUGetFileName(char *subject, char *ptonum, char *ptonend)
{
    char *ptr = subject, *iter = NULL, *result = NULL, *part = NULL;
 ...
 **/
	...
	iter++;
	while (isspace((int)*iter))
	    iter++;
	count = length = alflag = 0;
	while (iter[count] &&
	       (isalnum((int)iter[count])
		|| strchr(fnchars, iter[count]) != NULL)) {
	    if (isalpha((int)iter[count]))
		alflag++;
	    count++;
	}
	if (count < 4 || alflag == 0) {
	...
	while (*ptr && *ptr != 0x0a && *ptr != 0x0d && ptr != part) {
	    ...
	    while ((isalnum((int)*iter) || strchr(fnchars, *iter) != NULL ||
		    *iter == '/') && *iter && iter != ptonum
		   && *iter != '.') {
		if (isalpha((int)*iter))
		    alflag = 1;
		...
		/* hey, that's an ftp address */
		while (isalpha((int)*iter) || isdigit((int)*iter)
		       || *iter == '.')
		    iter++;
		ptr = iter;
		length = 0;
	...
	while (*ptr && !isalpha((int)*ptr))
	    ptr++;

	while ((isalnum((int)ptr[length])
		|| strchr(fnchars, ptr[length]) != NULL || ptr[length] == '/')
	       && ptr[length] && ptr + length != part
	       && ptr + length != ptonum)
	    length++;

	if (length) {
	    if (ptr[length] == '\0' || ptr[length] == 0x0a
		|| ptr[length] == 0x0d) {
		length--;
...
{
    char *where = NULL, *whend = NULL, temp[80], *ptr = NULL, *p2 = NULL;
    uufile *result;
...
*/
	...
	if ((result->filename = _FP_strdup(data->filename)) == NULL) {
	    UUMessage(uucheck_id, __LINE__, UUMSG_ERROR,
		      uustring(S_OUT_OF_MEMORY), strlen(data->filename) + 1);
	    *ret = UURET_NOMEM;
	    UUkillfile(result);
	    return NULL;
	}
	...
	if (result->filename == NULL || result->subfname == NULL) {
	    UUMessage(uucheck_id, __LINE__, UUMSG_ERROR,
	...
	if (data->uudet == B64ENCODED) {
		...
		UUMessage(uucheck_id, __LINE__, UUMSG_ERROR,
			  uustring(S_OUT_OF_MEMORY),
			  (result->filename) ?
			  (strlen(result->filename) + 1) : (strlen(temp) +
							    1));
		*ret = UURET_NOMEM;
		UUkillfile(result);
		return NULL;
...
int UUInsertPartToList(uufile * data)
{
	...
	if (data->data->flags & FL_SINGLE) {
	    ...
	    if (data->data->begin)
		iter->begin = (data->partno) ? data->partno : 1;
	    if (data->data->end)
		iter->end = (data->partno) ? data->partno : 1;
		...
		if (data->partno == fiter->partno) {
			...
			return UURET_NODATA;
		    else if (_FP_stristr(fiter->data->subject, "repost") !=
			     NULL
			     && _FP_stristr(data->data->subject,
					    "repost") == NULL)
			return UURET_NODATA;
    ...
    unew->begin =
	(data->data->begin) ? ((data->partno) ? data->partno : 1) : 0;
...
uulist *UUCheckGlobalList(void)
{
	...
	if (liter->state & UUFILE_OK) {
	    ...
	    continue;
		  ...
		  liter->uudet == PT_ENCODED) && (liter->flags & FL_SINGLE)) {
	...
	if (liter->uudet == B64ENCODED ||
	    liter->uudet == QP_ENCODED || liter->uudet == PT_ENCODED)
	...
	while (fiter != NULL) {
	    for (count = part + 1;
		 count < fiter->partno && miscount < MAXPLIST; count++)
		misparts[miscount++] = count;
	...
	if (havecount) {
	    if ((liter->haveparts =
		 (int *)malloc((havecount + 1) * sizeof(int))) != NULL) {
	...
	if (miscount) {
	    if ((liter->misparts =
		 (int *)malloc((miscount + 1) * sizeof(int))) != NULL) {
	...
	if ((flag & 4) == 0)
	    liter->state |= UUFILE_NODATA;
	if ((flag & 1) == 0)
	    liter->state |= UUFILE_NOBEGIN;
	if ((flag & 2) == 0)
	    liter->state |= UUFILE_NOEND;

	if ((flag & 7) == 7 && miscount == 0) {
	    liter->state = UUFILE_OK;
	}

	if ((uu_fast_scanning && (liter->flags & FL_PROPER) == 0)
	    || thesize <= 0)
	    liter->size = -1;
	else
	    liter->size = thesize;
