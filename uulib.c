...
char *uulib_id = "$Id: uulib.c,v 1.22 2001/06/06 18:21:47 fp Exp $";
...
int uu_dumbness = 2;		/* switch off the program's intelligence    */
...
int UUBusyPoll(void)
{
	...
	msecs =
	    1000 * (tv.tv_sec - uu_last_secs) + (tv.tv_usec -
						 uu_last_usecs) / 1000;
...
int UUEXPORT UUGetOption(int option, int *ivalue, char *cvalue, int clength)
{
    ...
    case UUOPT_USETEXT:
	if (ivalue)
	    *ivalue = uu_handletext;
	result = uu_handletext;
	break;
    case UUOPT_PREAMB:
	if (ivalue)
	    *ivalue = uu_usepreamble;
...
UUSetBusyCallback(void *opaque,
		  int (*func) _ANSI_ARGS_((void *, uuprogress *)), long msecs)
{
...
int UUEXPORT
UUSetFNameFilter(void *opaque, char *(*func) _ANSI_ARGS_((void *, char *)))
{
    uu_FNameFilter = func;
    uu_FFCBArg = opaque;
...
int UUEXPORT UULoadFile(char *filename, char *fileid, int delflag)
{
    ...
    _FP_strncpy(progress.curfile,
		(strlen(filename) > 255) ?
		(filename + strlen(filename) - 255) : filename, 256);
	...
	if ((loaded->subject && *(loaded->subject)) ||
	    ...
	    (loaded->filename && *(loaded->filename)) || (loaded->uudet)) {
	    UUMessage(uulib_id, __LINE__, UUMSG_MESSAGE,
		      ...
		      fload->partno,
		      (loaded->begin) ? "begin" : "",
		      (loaded->end) ? "end" : "", codenames[loaded->uudet]);
	}
...
int UUEXPORT UUDecodeFile(uulist * thefile, char *destname)
{
	...
	UUMessage(uulib_id, __LINE__, UUMSG_ERROR, uustring(S_NO_BIN_FILE));
	return UURET_IOERR;
    }
	...
	UUMessage(uulib_id, __LINE__, UUMSG_NOTE,
		  uustring(S_STRIPPED_SETUID), destname, (int)thefile->mode);
	thefile->mode &= 0777;
    }
    ...
    _FP_strncpy(progress.curfile,
		...
		(uugen_fnbuffer + strlen(uugen_fnbuffer) -
		 255) : uugen_fnbuffer, 256);
    progress.partno = 0;
    progress.numparts = 1;
	   ...
	   int (*func) _ANSI_ARGS_((void *, char *)))
{
    ...
    maxpos =
	thefile->thisfile->data->startpos + thefile->thisfile->data->length;

    while (!feof(inpfile) && (uu_fast_scanning || ftell(inpfile) < maxpos)) {
	...
	if (thefile->uudet == B64ENCODED && dd == B64ENCODED)
	    ...
	    break;
	else if ((thefile->uudet == UU_ENCODED
		  || thefile->uudet == XX_ENCODED)
...
int UUEXPORT UURenameFile(uulist * thefile, char *newname)
{
	...
	UUMessage(uulib_id, __LINE__, UUMSG_ERROR,
		  uustring(S_NOT_RENAME), oldname, newname);
