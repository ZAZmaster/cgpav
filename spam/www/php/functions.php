...
function show_white_black(&$error_msg, $dbh, $username)
{
	...
	$html .= "          <td>". ($row['preference'] == "whitelist_from" 
		    ? "Allow" : "Deny") . "</td>\n";
    	$html .= "          <td><a href='sa.php?delete=". $row['prefid'] 
	    ."'>Delete</a>";
	$html .= "&nbsp;<a href='sa.php?edit=". $row['prefid'] 
	    ."'>Edit</a></td>\n";
	$html .= "        </tr>\n";    	    
    }
...
function create_spam_rule($username, $action = 5) 
{
    global $CONF;
    $spam_folder_found = 0;
    ...
    if ($action == 2) {
	$action_array = array(
	    array('Reject with', 'You seems to be a spammer. This user rejected your message.')
	);							  
    }
    // action discard
    else if ($action == 3)
	$action_array = array(array('Discard'));
    // action addheader
    else {
	$action_array = array(
    ...
    if ($CONF['debug'])
	echo "SetAccountRules: ". $cli->getErrMessage() ."<br>";
	
    if ($action == 5) {
	// add the spam folder if it doesn't exist
	...
	if (!$spam_folder_found) {
	    ...
	    if ($CONF['debug'])
		echo "Create, SetAccountSubscription: ". $cli->getErrMessage() ."<br>";
	}
    }    
...
function remove_spam_rule($username) 
{
	...
	// cgpro doesn't like missed keys
	sort($Rules);
	$cli->SetAccountRules($username, $Rules);    		
	...
	if ($CONF['debug'])
	    echo "SetAccountSubscription: ". $cli->getErrMessage() ."<br>";
	...
	if ($CONF['debug'])
	    echo "DeleteMailbox: ". $cli->getErrMessage() ."<br>";
    }    
...
function show_tests(&$error_msg, $dbh, $username)
{
	...
	$html .= "          <td align=\"right\">". $row['value'] ."</td>\n";
    	$html .= "          <td><a href='sa.php?deltest=". $row['prefid'] 
	    ."'>Delete</a>";
	$html .= "&nbsp;<a href='sa.php?edittest=". $row['prefid'] 
	    ."'>Edit</a></td>\n";
	$html .= "        </tr>\n";    	    
    }
